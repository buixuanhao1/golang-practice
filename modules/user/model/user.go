...
// Scan dùng để convert từ DB -> struct (custom type)
...
// Hàm Value giúp GORM lưu giá trị chuỗi xuống database thay vì số int
...
// MarshalJSON để convert UserRole thành chuỗi khi trả JSON ra ngoài (ví dụ API)
...
type User struct {
	...
	Status          int      `json:"status" gorm:"colum:status"`
}
